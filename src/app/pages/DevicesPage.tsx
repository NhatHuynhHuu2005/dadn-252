...
export function DevicesPage() {
  ...
  const canManageDevices = !isWorker;
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setError('Không the tải du lieu. Vui lòng thu lai.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const toggleActuator = async (deviceId: string) => {
    ...
    setDeviceList(prev => {
      ...
      if (!device || !['pump', 'valve', 'fan'].includes(device.type)) return prev;

      const nextValue = device.lastValue === 1 ? 0 : 1;

      deviceApi.update(deviceId, { ...device, lastValue: nextValue, status: 'online' })
        .catch(err => {
          console.error("Lỗi khi điều khiển thiết bị:", err);
          alert("Lỗi mạng: Không thể lưu trạng thái thiết bị vào Database!");
        });

      return prev.map(d => 
        d.id === deviceId ? { ...d, lastValue: nextValue, status: 'online' } : d
      );
    });
  };
  const handleAddDevice = async () => {
    if (!canManageDevices) return;
    if (!addForm.name) {
      setAddError('Vui lòng nhap ten thiet bi');
      return;
    }
    if (!addForm.fieldId) {
      setAddError('Vui lòng chon canh dong');
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {wsConnected ? (
            <span className="status-badge online"><Wifi className="w-4 h-4" /> Socket kết nối</span>
          ) : (
            <span className="status-badge error"><WifiOff className="w-4 h-4" /> Mất kết nối</span>
          )}
          {canManageDevices && (
              ...
              {isActuator ? (
                <div className="flex items-center justify-between mt-4">
                  ...
                  {canManageDevices ? (
                    ...
                    <span className="text-xs text-gray-400">Chi xem</span>
                  )}
                </div>
              ) : (
                <div className="mt-2">
          ...
          <div className="modal-content" style={{ maxWidth: '680px' }}>
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {canManageDevices && (
                  <button onClick={() => { setDeleteTarget(selectedDevice.id); }} className="action-btn delete" title="Xóa thiet bi">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-[#f8faf8] rounded-xl p-4 text-center border border-green-50">
                ...
                <p className="text-sm text-green-600 mt-1">{wsConnected ? 'Đã kết nối' : 'Mất kết nối'}</p>
              </div>
            </div>
            ...
            <div className="mt-4 p-4 bg-green-50 rounded-xl border border-green-100">
              ...
              <p className="text-xs text-green-600 mt-2">Dữ liệu được cập nhật từ WebSocket và sensor history API</p>
            </div>
          </div>
        </div>
      )}
